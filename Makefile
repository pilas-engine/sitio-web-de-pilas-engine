...
deploy_a_produccion:
	...
	@touch _site/.static
	@ls -la _site/
	@cd _site; git init; git add .; git config user.email "hugoruscitti@gmail.com"; git config user.name "Hugo Ruscitti"; git commit -am 'rebuild' --allow-empty; git remote add dokku dokku@pilas-engine.com.ar:00-sitio-web-de-pilas-engine; git push dokku master -f
	rm -rf _site
